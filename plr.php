...
if(isset($_POST["submit4"]))
{
	...
	mysqli_set_charset($dbc, 'utf8');
		...
		$query="select * from Player where PlayerName='$name' and Password='$pass'";
		...
		if($num != 0)
		{
			...
			echo '<h2>Hello '.$name.'. Welcome to the '.$team.' team portal!</h2></font><font color="turqoise"><h3><u>COACH DETAILS</u></h3>';
			
			$query="select * from Coach where TeamName='$team'";	
			$result = @mysqli_query ($dbc, $query);
			$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
			echo '<font color="fire engine red"><table border="1" cellspacing="5" cellpadding="5" ><th>Coach ID</th><th>Coach Name</th><th>Address</th><th>City</th><th>Contact Number</th>';
			echo'<tr><td><center>'.$row['CoachID'].'</center></td><td>'.$row['CoachName'].'</td><td>'.$row['Address'].'</td><td>'.$row['City'].'</td><td><center>'.$row['ContactNum'].'</center></td></tr>';
